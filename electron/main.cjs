...
function defaultDirectory() {
  try {
    const codexDir = path.join(app.getPath("documents"), "Codex");
...
function runClaude({ conversationId, prompt, attachments }) {
  ...
  const args = [
    ...
    "你正在 Claude to Code 桌面壳中运行。请以 Claude Code/Claude 的身份回答，不要自称 Kiro，也不要引用 Kiro 开发环境的身份说明，除非用户明确询问 Kiro。输出给用户的正文不要使用 Markdown 标题井号 #，也不要使用星号 * 或 ** 做加粗/斜体；需要分段时直接写自然段，列表优先用数字编号或普通短横线。",
    "--output-format",
    "stream-json",
  ...
  const child = spawn(
    ...
    {
      cwd: conversation.directory || defaultDirectory(),
